...
class WordbridgeModelEntries extends JModelLegacy
{
    ...
    function loadEntries($page = 1, $blogInfo, $nocache = false)
    {
        ...
        $clearSql = sprintf('DELETE FROM #__com_wordbridge_cache WHERE blog_uuid = %s AND UNIX_TIMESTAMP(%s) < %d', $db->quote($blogInfo['uuid'], true), $db->quoteName('update_time'), $expiredTime);
