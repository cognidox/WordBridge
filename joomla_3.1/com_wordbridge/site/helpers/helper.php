...
class WordbridgeHelper {
    ...
    public static function storeBlogEntries( $entries, $blog_uuid )
    {
        ...
        foreach ( $entries as $entry )
        {
            ...
            $post_query = sprintf( 
                ...
                //$db->quote( JFactory::getDate( $entry['date'] )->format( '%F %T %Z' ), true ),
                $db->quote( JFactory::getDate( $entry['date'] )->format( 'Y-m-d H:i:s T' ), true ),
    ...
    public static function wordBridgeStrftime( $format, $time = null, $local = false )
    {
        ...
        return JFactory::getDate( $time )->format( $format, $local );
    }
}
